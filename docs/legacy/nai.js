...
(function(){
  ...
  function extractNovelAIVersion(source) {
    const match = /NovelAI Diffusion V([1-9]\d*(?:\.\d+)?)(?=\s|$)/i.exec(String(source || ''));
  ...
  function formatVersion(source, usedDescriptionFallback, lang) {
    ...
    if (source.includes('NovelAI Diffusion V4.5 B5A2A797')) {
      ...
      const version = extractNovelAIVersion(source);
      if (version) label = `NAI${version}`;
    }
