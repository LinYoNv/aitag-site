...
- **详情页**：单图放大、多图三列展示；每张图完整生成参数（prompt/负向/sampler/seed/底模/LoRA）；**画师(artist)解析**（NovelAI 新旧格式、权重、大括号保留、**NAI v4/v5 加权画师串**）；Prompt/Negative/画师 三框复制按钮（uc 为纯画师列表时按「排除画师」呈现）；**点赞(👍)/收藏(⭐)/浏览量(👁)**（浏览量 10 分钟窗口去重防刷）
